...
1st off, compilation tests, typescript, babel, webpack. these are good examples of when they fail, it's a pretty big user facing deal, usually it means your code wouldn't even be deployable it has syntax errors so bad or referencing names that don't exist
it doesn't do so hot on the second part though, when they pass you're not exactly confident your code does what you want it to do. it's more like a bare minimum

linters, eslint tslint etc, some of what these check are good user-facing problems, perhaps using a variable that doesn't exist or catching sneaky mistakes like variable shadowing or ===, but a lot of it is also just consistency, not exactly high signal user-facing issues

unit tests, are where we start to get more interesting. these might be running with mocha or jest and exercise a single component or small part of your site in isolation. they might ensure that a component passes the right props to the right children for example, which is a helpful signal but just because they pass doesn't mean everything is going to work smoothly together as a unit

and that's where we get to integration tests, they have many names (integration, e2e, whatever you call it) but I'm talking about here is using the application in the way that is closest to how your end user is going to use it, for a website that means loading it in a real browsser, and trying to use it!
...
- Automatically downloads a compatible copy of Chromium or Firefox, i.e. batteries included
